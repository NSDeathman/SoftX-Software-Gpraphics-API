    ...
    const ConstantBuffer& cb)
{
    ...
    // Удвоенная площадь треугольника (для culling и барицентрических координат)
    ...
    for (int y = iMinY; y <= iMaxY; ++y)
    {
        for (int x = iMinX; x <= iMaxX; ++x)
        {
            ...
            // Проверка принадлежности пикселя треугольнику (с учётом знака площади)
...
void RasterizerScalar::RasterizeTriangleTile(
    const VertexOutput& v0,
    const VertexOutput& v1,
    const VertexOutput& v2,
    const RasterizerState& state,
