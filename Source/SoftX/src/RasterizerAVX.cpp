    ...
    const ConstantBuffer& cb)
{
    ...
    // Размножаем атрибуты вершин на 8 элементов
    ...
    for (int y = iMinY; y <= iMaxY; ++y)
    {
        ...
        int x;
        // Обрабатываем блоки по 8 пикселей
        for (x = iMinX; x <= iMaxX - 7; x += 8)
        {
            // X-координаты для 8 пикселей: x+0.5, x+1.5, ..., x+7.5
            __m256 baseX = _mm256_set_ps(x + 7.5f, x + 6.5f, x + 5.5f, x + 4.5f,
                                          x + 3.5f, x + 2.5f, x + 1.5f, x + 0.5f);

            // Edge-функции для всех 8 пикселей
            ...
            // Загрузка текущих значений глубины для 8 пикселей
            int idx0 = y * width + x;
            ...
            // Сохраняем интерполированные значения в массивы для последующего вызова шейдера
            ...
            // Обрабатываем каждый пиксель, прошедший тест глубины
            for (int i = 0; i < 8; ++i)
            {
                if (depthMask & (1 << i))
                {
        ...
        // Скалярный доводчик для оставшихся пикселей (менее 8 в строке)
        for (; x <= iMaxX; ++x)
        {
...
void RasterizerAVX::RasterizeTriangleTile(
    const VertexOutput& v0,
    const VertexOutput& v1,
    const VertexOutput& v2,
    const RasterizerState& state,
    ...
    int2 tileMax)
{
    PROFILE_SCOPE("RasterizerAVX::RasterizeTriangleTile");
