    ...
    const ConstantBuffer& cb)
{
    ...
    // SSE-регистры с атрибутами вершин (размноженные по 4 элементам)
    ...
    for (int y = iMinY; y <= iMaxY; ++y)
    {
        ...
        for (x = iMinX; x <= iMaxX - 3; x += 4)
        {
            ...
            // Загрузка текущих значений глубины для 4 пикселей
            int idx0 = y * width + x;
        ...
        // Скалярный доводчик для оставшихся пикселей (менее 4 в строке)
        for (; x <= iMaxX; ++x)
        {
...
void RasterizerSSE::RasterizeTriangleTile(
    const VertexOutput& v0,
    const VertexOutput& v1,
    const VertexOutput& v2,
    const RasterizerState& state,
