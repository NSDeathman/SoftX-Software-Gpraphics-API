...
{
	m_tiles.clear();
	int tilesX = (width + m_tileSize - 1) / m_tileSize;
	int tilesY = (height + m_tileSize - 1) / m_tileSize;
	for (int ty = 0; ty < tilesY; ++ty)
	{
		for (int tx = 0; tx < tilesX; ++tx)
		{
			int2 min(tx * m_tileSize, ty * m_tileSize);
			int2 max(std::min((tx + 1) * m_tileSize - 1, width - 1), std::min((ty + 1) * m_tileSize - 1, height - 1));
...
void Device::binTriangles(const std::vector<VertexOutput>& verts, const std::vector<int3>& triangles)
{
	...
	for (int triIdx = 0; triIdx < (int)triangles.size(); ++triIdx)
	{
		...
		int tileX0 = std::max(0, (int)(minX / m_tileSize));
		int tileY0 = std::max(0, (int)(minY / m_tileSize));
		int tileX1 = std::min((int)(maxX / m_tileSize), (m_currentRT->width() - 1) / m_tileSize); // исправлено!
		int tileY1 = std::min((int)(maxY / m_tileSize), (m_currentRT->height() - 1) / m_tileSize);

#ifdef DEBUG_TILES
		// Отладка: выводим для первых 5 треугольников
		if (triIdx < 5)
		{
			char buf[256];
			sprintf_s(buf, "Tri %d: bbox=(%.1f,%.1f)-(%.1f,%.1f) tileX=[%d,%d] tileY=[%d,%d]\n", triIdx, minX, minY,
					  maxX, maxY, tileX0, tileX1, tileY0, tileY1);
		...
		for (int ty = tileY0; ty <= tileY1; ++ty)
		{
			...
			{
				int tileIdx = ty * ((m_currentRT->width() + m_tileSize - 1) / m_tileSize) + tx; // исправлено!
...
{
	int width = m_currentRT->width();
	int height = m_currentRT->height();
	...
	// Пересекаем с тайлом (целые координаты пикселей)
	...
	if (m_cullMode == CullMode::Back && area2 < 0)
		return;
	if (m_cullMode == CullMode::Front && area2 > 0)
		return;
	...
	for (int y = iMinY; y <= iMaxY; ++y)
	{
		for (int x = iMinX; x <= iMaxX; ++x)
		{
			...
			if (z < m_depthBuffer.at(idx))
			{
				...
				m_currentRT->set_pixel(int2(x, y), finalColor);
			}
		}
	}
}
...
void Device::RasterizeTriangleTileSSE(const VertexOutput& v0, const VertexOutput& v1, const VertexOutput& v2, int2 tileMin, int2 tileMax)
{
	...
	// ---------- SSE-часть ----------
	{
		// Предвычисляем константы (без изменений)
		...
		for (int y = iMinY; y <= iMaxY; ++y)
		{
			...
			// Определяем диапазоны для SSE-блоков и остатков
			int xStart = iMinX;
			int xEnd = iMaxX;

			// Округляем начало блоков вверх до ближайшего кратного 4
			int xBlockStart = (xStart + 3) & ~3;
			// Округляем конец блоков вниз до кратного 4
			int xBlockEnd = xEnd & ~3;

			// 1. Скалярная обработка левого остатка (xStart ... xBlockStart-1)
			...
			// 2. SSE-обработка полных блоков (xBlockStart ... xBlockEnd-1) с шагом 4
			for (int x = xBlockStart; x < xBlockEnd; x += 4)
			{
				...
				__m128 z =
					_mm_add_ps(_mm_add_ps(_mm_mul_ps(alpha, v0z), _mm_mul_ps(beta, v1z)), _mm_mul_ps(gamma, v2z));

				// Интерполяция цвета
				__m128 r =
					_mm_add_ps(_mm_add_ps(_mm_mul_ps(alpha, v0cr), _mm_mul_ps(beta, v1cr)), _mm_mul_ps(gamma, v2cr));
				__m128 g =
					_mm_add_ps(_mm_add_ps(_mm_mul_ps(alpha, v0cg), _mm_mul_ps(beta, v1cg)), _mm_mul_ps(gamma, v2cg));
				__m128 b =
					_mm_add_ps(_mm_add_ps(_mm_mul_ps(alpha, v0cb), _mm_mul_ps(beta, v1cb)), _mm_mul_ps(gamma, v2cb));
				__m128 a =
					_mm_add_ps(_mm_add_ps(_mm_mul_ps(alpha, v0ca), _mm_mul_ps(beta, v1ca)), _mm_mul_ps(gamma, v2ca));

				// Интерполяция UV
				__m128 u =
					_mm_add_ps(_mm_add_ps(_mm_mul_ps(alpha, v0u), _mm_mul_ps(beta, v1u)), _mm_mul_ps(gamma, v2u));
				__m128 v =
					_mm_add_ps(_mm_add_ps(_mm_mul_ps(alpha, v0v), _mm_mul_ps(beta, v1v)), _mm_mul_ps(gamma, v2v));
				...
				for (int i = 0; i < 4; ++i)
				{
					...
					if (depthMask & bit)
					{
						...
						float4 finalColor = m_pixelShader(frag, m_constant_buffer);
						m_currentRT->set_pixel(int2(px, py), finalColor);
					}
				}
			}

			// 3. Скалярная обработка правого остатка (xBlockEnd ... xEnd)
...
#ifdef DEBUG_TILES
	// === Залить тайл красным, если в нём есть треугольники ===
	if (!tile.triangleIndices.empty())
	{
		// Рисуем красную рамку по периметру тайла
		for (int x = tile.min.x; x <= tile.max.x; ++x)
		{
			m_currentRT->set_pixel(int2(x, tile.min.y), float4(1, 0, 0, 1));
			m_currentRT->set_pixel(int2(x, tile.max.y), float4(1, 0, 0, 1));
		}
		for (int y = tile.min.y; y <= tile.max.y; ++y)
		{
			m_currentRT->set_pixel(int2(tile.min.x, y), float4(1, 0, 0, 1));
			m_currentRT->set_pixel(int2(tile.max.x, y), float4(1, 0, 0, 1));
		}
	}
	// === конец отладки ===
#endif
