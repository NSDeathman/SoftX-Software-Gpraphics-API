...
public:
    Framebuffer(int2 size)
        : m_width(size.x), m_height(size.y), m_pixels(size.x* size.y, 0xFF000000)
    {
    }
    ...
    void set_pixel(int2 coords, const float4& color) {
    ...
    void present(HDC hdc, int2 dstPos = int2(0, 0), int2 dstSize = int2(-1, -1)) const {
        ...
        SetDIBitsToDevice(hdc, dstPos.x, dstPos.y, dstW, dstH, 0, 0, 0, m_height,
            m_pixels.data(), &bmi, DIB_RGB_COLORS);
    }
